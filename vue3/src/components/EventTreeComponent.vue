      ...
      <div class="columns">
        ...
        <div class="column">
          <div v-if="selectedEvent != null">
            <div
              class="track"
              :class="track.selected ? 'selected' : ''"
              v-for="track in selectedEvent.tracks"
              :key="track.id"
              @click="selectTrack(selectedEvent, track)"
            >
              {{ track.label }}
            </div>
          </div>
...
export default defineComponent({
  ...
  computed: {
    ...
    selectedEvent() {
      ...
      if (filteredResult == null || filteredResult.length == 0) return null
  ...
  methods: {
    ...
    selectTrack(
      ...
      selectedTrack: SelectionTreeEventTrack
    ) {
      let found = false
